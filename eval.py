...
directory = './model/' + args.env_name + '/'
result_directory = "./results/" + args.env_name + '/'
...
def main():
    ...
    icm_agent = ICM_DQNAgent(in_channels=state_dim, action_dim=action_dim, device=device, lr=0.005,
                             ...
                             use_extrinsic=use_extrinsic,env_name=args.env_name)
    ep_r = 0
    if args.mode == 'test':
        agent.load()
        attack_type = "sarsa"
        config_path = "./config/" + args.env_name + ".json"
        attack = attack_agent(agent, attack_type=attack_type, config_path=config_path)
        reward_list = []
        for i in range(args.test_iteration):
            ...
            for t in count():
                ...
                if done or t >= 2000:
                    ...
                    ep_r = 0
                    break
                state = attack.eval_step(next_state)
        print("avg {}, fangcha {}".format(sum(reward_list) / 10, np.std(reward_list,ddof=1)))
        ...
        for i in range(args.max_episode):
            ...
            for t in count():
                action = agent.select_action(state)
                adv_action = icm_agent.select_action(state_tensor=state, action=action).detach().cpu().numpy()

                sum_action = action + adv_action

                action = (sum_action + np.random.normal(0, args.exploration_noise, size=env.action_space.shape[0])).clip(
